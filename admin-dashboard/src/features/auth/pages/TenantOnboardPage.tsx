...
export function TenantOnboardPage() {
  ...
  // Read prefilled info if forwarded from Google login
  ...
  // Form field states (prefilled with Google email if available)
  const [formData, setFormData] = useState({
    company_name: "",
    subdomain: "",
    admin_email: locationState?.prefilledEmail || "",
    password: "",
    confirm_password: "",
  });
  const [industry, setIndustry] = useState<Industry | "">("");
  const isGooglePrefilled = Boolean(locationState?.prefilledEmail);
  
  // Google OAuth popup handler for 1-click pre-fill
      ...
      try {
        ...
        if (googleUser?.email) {
          setFormData((prev) => ({
            ...
            company_name: prev.company_name || (googleUser.name ? `${googleUser.name}'s Freight` : ""),
          }));
          toast.success(`Google profile verified! Verified email: ${googleUser.email}`);
        }
      } catch (err) {
        ...
        <div className="flex flex-col w-full max-w-md mx-auto">
          ...
          <div className="glass-panel p-6 md:p-8 rounded-2xl shadow-xl space-y-5 border border-slate-800">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">Set up your logistics hub & activate your 30-day free trial.</p>
            </div>
            ...
            {isGooglePrefilled && (
              <div className="bg-teal-500/10 border border-teal-500/30 rounded-xl p-3 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <span className="text-[10px] text-teal-400 font-bold uppercase tracking-wider block">Google Account Verified</span>
                  <p className="text-xs text-slate-200 font-mono truncate">{formData.admin_email}</p>
                </div>
              </div>
            )}
            ...
            {!isGooglePrefilled && (
              <>
                ...
                <button 
                  ...
                  className="w-full bg-slate-900 hover:bg-slate-800 border border-slate-700/80 text-slate-200 font-semibold py-3 rounded-xl transition-all flex items-center justify-center gap-3 cursor-pointer"
                >
                  ...
                  <span>Pre-fill with Google</span>
                </button>
              </>
            )}
